...
export const FeishuBotPanel = {
  ...
  methods: {
    async load() {
      ...
      if (this.session) this.sessionLink = await api(withWorkspace(`/api/sessions/${this.session.id}/feishu-bot`, wid));
    },
    async save() {
      await this.ctx.run('正在保存协同入口', async () => {
    ...
    async link(enabled) {
      ...
      this.sessionLink = await api(withWorkspace(`/api/sessions/${this.session.id}/feishu-bot`, this.ctx.state.workspaceId), {
        method: 'PUT', body: { enabled, chat_id: this.form.receive_id },
      });
      this.ctx.toast('', enabled ? '当前会话已连接飞书群' : '当前会话已断开飞书群');
    },
    async test() {
      await this.ctx.run('正在发送协同入口测试消息', () => api(withWorkspace('/api/feishu-bot/test', this.ctx.state.workspaceId), { method: 'POST' }));
    },
  },
  template: `<section class="workspace-page"><header class="surface-header page-heading"><div><h1>协同渠道</h1><p>配置飞书机器人并将分析会话绑定到群聊，实现跨端提问与结果同步。</p></div><StatusPill v-if="status" :status="connectionState"/></header>
...
export const SettingsPanel={components:{DataTable,EmptyState,Icon,StatusPill},props:{ctx:Object},data:()=>({tab:localStorage.getItem('meridian-settings-tab')||'members',members:[],providers:[],mcp:[],connectors:[],memories:[],audit:[],compute:null,skills:[],feishuBot:null,feishuChats:[],sessionBot:null,inviteResult:null,memberForm:{email:'',role:'analyst'},providerForm:{name:'OpenAI Compatible',base_url:'https://api.openai.com/v1',model:'gpt-4.1-mini',api_key:'',temperature:0.2},mcpForm:{name:'工具服务',transport:'streamable-http',url:'',command:'',argsText:'[]',headersText:'{}',envText:'{}'},connectorForm:{name:'团队通知',type:'webhook',url:'',host:'',port:587,username:'',password:'',sender:'',recipient:'',use_tls:true,app_id:'',app_secret:'',receive_id:'',receive_id_type:'chat_id',verification_token:''},botForm:{enabled:false,app_id:'',app_secret:'',event_verification_token:'',inbound_transport:'long_connection',receive_id_type:'chat_id',receive_id:''},memoryForm:{title:'',content:'',scope:'workspace'},skillForm:{name:'',description:'',instruction:''}}),mounted(){this.load()},watch:{tab(value){localStorage.setItem('meridian-settings-tab',value)}},methods:{async load(){const wid=this.ctx.state.workspaceId;const [wm,p,m,c,mem,a,comp,s]=await Promise.all([api(`/api/workspaces/${wid}/members`),api('/api/providers'),api(withWorkspace('/api/mcp/servers',wid)),api(withWorkspace('/api/connectors',wid)),api(withWorkspace('/api/memories',wid)),api(withWorkspace('/api/audit?limit=100',wid)),api(withWorkspace('/api/compute/status',wid)),api(withWorkspace('/api/skills',wid))]);this.members=wm.items;this.providers=p.items;this.mcp=m.items;this.connectors=c.items;this.memories=mem.items;this.audit=a.items;this.compute=comp;this.skills=s.items;try{const bot=await api(withWorkspace('/api/feishu-bot',wid));this.feishuBot=bot.connection;this.botForm={...this.botForm,...bot.connection,app_secret:'',event_verification_token:''};const sid=this.ctx.activeSession()?.id;if(sid)this.sessionBot=await api(withWorkspace(`/api/sessions/${sid}/feishu-bot`,wid));}catch{}},async addMember(){if(!this.memberForm.email.trim())return;await this.ctx.run('正在添加成员',async()=>{this.inviteResult=null;try{await api(`/api/workspaces/${this.ctx.state.workspaceId}/members`,{method:'POST',body:this.memberForm})}catch(error){if(error.status!==404)throw error;const invitation=await api(`/api/workspaces/${this.ctx.state.workspaceId}/invitations`,{method:'POST',body:this.memberForm});this.inviteResult={...invitation,invitation_url:new URL(invitation.registration_url,window.location.origin).href}}this.memberForm.email='';await this.load()})},async setMemberRole(item){await api(`/api/workspaces/${this.ctx.state.workspaceId}/members/${item.user_id}`,{method:'PATCH',body:{role:item.role}});this.ctx.toast('','成员角色已更新')},async removeMember(item){if(!window.confirm(`移除成员 ${item.name||item.email}？`))return;await api(`/api/workspaces/${this.ctx.state.workspaceId}/members/${item.user_id}`,{method:'DELETE'});await this.load()},async saveProvider(){await this.ctx.run('正在保存模型配置',async()=>{await api('/api/providers',{method:'POST',body:this.providerForm});this.providerForm.api_key='';await this.load()})},async testProvider(item){await this.ctx.run('正在进行真实模型请求',async()=>{const result=await api(`/api/providers/${item.id}/test`,{method:'POST'});this.ctx.toast(`${result.result.model} · ${result.result.latency_ms} ms`,'模型连接正常')})},async saveMcp(){let headers={},env={},args=[];try{headers=JSON.parse(this.mcpForm.headersText||'{}');env=JSON.parse(this.mcpForm.envText||'{}');args=JSON.parse(this.mcpForm.argsText||'[]')}catch{return this.ctx.fail(new Error('请求头、环境变量和参数必须是 JSON'));}await api('/api/mcp/servers',{method:'POST',body:{...this.mcpForm,headers,env,args,workspace_id:this.ctx.state.workspaceId}});await this.load()},async testMcp(item){await this.ctx.run('正在握手并发现工具',async()=>{const result=await api(`/api/mcp/servers/${item.id}/test`,{method:'POST'});this.ctx.toast(`发现 ${result.result.tools.length} 个工具`,'MCP 已连接');await this.load()})},async saveConnector(){await api('/api/connectors',{method:'POST',body:{...this.connectorForm,workspace_id:this.ctx.state.workspaceId}});this.connectorForm.password='';this.connectorForm.app_secret='';await this.load()},async testConnector(item){await this.ctx.run('正在发送测试消息',async()=>{await api(`/api/connectors/${item.id}/test`,{method:'POST'});this.ctx.toast('测试消息已送达','通知连接正常')})},async saveFeishuBot(){await api('/api/feishu-bot',{method:'PUT',body:this.botForm});this.botForm.app_secret='';this.botForm.event_verification_token='';await this.load()},async loadFeishuChats(){const result=await api('/api/feishu-bot/chats');this.feishuChats=result.chats;if(!this.botForm.receive_id&&result.chats.length)this.botForm.receive_id= result.chats[0].chat_id},async linkFeishu(enabled){const sid=this.ctx.activeSession()?.id;if(!sid)return;this.sessionBot=await api(`/api/sessions/${sid}/feishu-bot`,{method:'PUT',body:{enabled,chat_id:this.botForm.receive_id}});this.ctx.toast('',enabled?'当前会话已连接飞书':'当前会话已断开飞书')},async addMemory(){if(!this.memoryForm.title.trim())return;await api('/api/memories',{method:'POST',body:{...this.memoryForm,workspace_id:this.ctx.state.workspaceId}});this.memoryForm={title:'',content:'',scope:'workspace'};await this.load()},async removeMemory(item){await api(`/api/memories/${item.id}`,{method:'DELETE',body:{confirm:true}});await this.load()},async addSkill(){await api('/api/skills',{method:'POST',body:{...this.skillForm,workspace_id:this.ctx.state.workspaceId}});this.skillForm={name:'',description:'',instruction:''};await this.load()},async editSkill(item){const instruction=window.prompt('修改执行指令；保存后会生成新的候选版本',item.instruction||'');if(instruction===null||!instruction.trim())return;await this.ctx.run('正在创建 Skill 候选版本',async()=>{await api(`/api/skills/${item.id}`,{method:'PATCH',body:{instruction}});await this.load()})},async evaluateSkill(item){await this.ctx.run('正在执行权限合同测试',async()=>{await api(`/api/skills/${item.id}/evaluate`,{method:'POST',body:{cases:[{input:'按技能说明完成一次最小任务',required_tools:item.allowed_tools||[],forbidden_tools:[]}]}});await this.load()})},async publishSkill(item){await this.ctx.run('正在审批发布 Skill',async()=>{await api(`/api/skills/${item.id}/publish`,{method:'POST'});await this.load()})},async deprecateSkill(item){await this.ctx.run('正在停用 Skill',async()=>{await api(`/api/skills/${item.id}/deprecate`,{method:'POST'});await this.load()})},async rollbackSkill(item){const version=Number(window.prompt('回滚到哪个历史版本？',String(Math.max(1,(item.version||1)-1))));if(!Number.isInteger(version)||version<1)return;await this.ctx.run('正在生成回滚候选版本',async()=>{await api(`/api/skills/${item.id}/rollback`,{method:'POST',body:{version}});await this.load()})}},template:`
<section class="workspace-page">
<header class="surface-header page-heading"><div><h1>平台设置</h1><p>管理成员权限、模型与工具服务、通知渠道、分析能力和运行审计。</p></div></header>
<div class="settings-layout"><nav class="settings-nav"><button :class="{active:tab==='members'}" @click="tab='members'"><Icon name="users"/>成员与权限</button><button :class="{active:tab==='models'}" @click="tab='models'"><Icon name="brain"/>模型服务</button><button :class="{active:tab==='mcp'}" @click="tab='mcp'"><Icon name="bolt"/>MCP 工具</button><button :class="{active:tab==='connectors'}" @click="tab='connectors'"><Icon name="workflow"/>通知连接</button><button :class="{active:tab==='memory'}" @click="tab='memory'"><Icon name="book"/>长期记忆</button><button :class="{active:tab==='skills'}" @click="tab='skills'"><Icon name="chart"/>分析技能</button><button :class="{active:tab==='compute'}" @click="tab='compute'"><Icon name="database"/>计算资源</button><button :class="{active:tab==='audit'}" @click="tab='audit'"><Icon name="table"/>审计日志</button></nav><main class="settings-content">
    <section v-if="tab==='members'"><div class="section-heading"><h2>成员与数据权限</h2><p>业务分析员可使用已发布的数据空间；数据管理员维护连接、指标与知识；只读用户不可执行分析。</p></div><div class="setting-list"><article v-for="item in members" :key="item.user_id"><div><b>{{ item.name||item.email||item.user_id }}</b><small>{{ item.email||'本地运行身份' }}</small></div><select v-model="item.role" :disabled="ctx.state.workspaceRole!=='owner'" @change="setMemberRole(item)"><option value="owner">工作空间所有者</option><option value="editor">数据管理员</option><option value="analyst">业务分析员</option><option value="viewer">只读用户</option></select><button v-if="ctx.state.workspaceRole==='owner'" class="icon-button danger" @click="removeMember(item)"><Icon name="close"/></button></article><EmptyState v-if="!members.length" icon="users" title="本地单用户模式" text="启用企业登录后可在此管理成员角色。"/></div><div v-if="ctx.state.workspaceRole==='owner'" class="settings-card"><h3>添加或邀请成员</h3><div class="form-grid"><label><span>企业邮箱</span><input v-model.trim="memberForm.email" type="email" placeholder="name@company.com"></label><label><span>角色</span><select v-model="memberForm.role"><option value="analyst">业务分析员</option><option value="viewer">只读用户</option><option value="editor">数据管理员</option><option value="owner">工作空间所有者</option></select></label></div><button class="button button--primary" :disabled="!memberForm.email" @click="addMember">添加或生成邀请</button><div v-if="inviteResult" class="readiness-banner"><div><Icon name="check"/><span><b>邀请链接已生成</b><small>{{ inviteResult.invitation_url }}（有效期 24 小时，请安全交付）</small></span></div></div></div></section>
    ...
    <section v-if="tab==='skills'"><div class="section-heading"><h2>分析技能</h2><p>候选版本必须经过确定性权限合同测试与所有者审批，发布后才会进入正式分析。</p></div><div class="setting-list"><article v-for="item in skills" :key="item.id"><div><b>{{ item.name }} <small>v{{ item.version||1 }}</small></b><small>{{ item.description }} · {{ item.latest_evaluation_status||'未测试' }}</small></div><StatusPill :status="item.status||'built_in'" :label="item.status||'内置'"/><div v-if="item.status" class="row-actions"><button class="button button--small" @click="editSkill(item)">新版本</button><button class="button button--small" @click="evaluateSkill(item)">测试</button><button v-if="item.latest_evaluation_status==='PASS' && item.status!=='published'" class="button button--small" @click="publishSkill(item)">审批发布</button><button v-if="item.status==='published'" class="button button--small" @click="deprecateSkill(item)">停用</button><button v-if="(item.version||1)>1" class="button button--small" @click="rollbackSkill(item)">回滚</button></div></article></div><div class="settings-card"><h3>添加候选技能</h3><div class="form-grid"><label><span>名称</span><input v-model="skillForm.name"></label><label><span>说明</span><input v-model="skillForm.description"></label><label class="span-2"><span>执行指令</span><textarea v-model="skillForm.instruction"></textarea></label></div><button class="button button--primary" @click="addSkill">创建候选版本</button></div></section>
    <section v-if="tab==='compute' && compute"><div class="section-heading"><h2>本机运行资源</h2><p>仅用于小型有界分析的诊断；大数据作业由 Trino / Livy 引擎配置承载。</p></div><div class="metric-strip"><div><small>CPU 逻辑核心</small><b>{{ compute.local.cpu_count }}</b></div><div><small>加速后端</small><b>{{ compute.local.gpu.backend || '未启用' }}</b></div><div><small>Python</small><b>{{ compute.local.python }}</b></div></div><div class="settings-card"><h3>运行平台</h3><p>{{ compute.local.platform }}</p></div></section>
